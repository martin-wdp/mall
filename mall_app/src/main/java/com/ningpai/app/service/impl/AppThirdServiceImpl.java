 ...
 * @version V1.0
 */
@Service("AppThirdService")
public class AppThirdServiceImpl implements AppService {

    /** 记录日志对象 */
    private static final Logger LOGGER = Logger.getLogger(AppThirdServiceImpl.class);
    ...
    public Bundle downloadAndInstallApp(Long id, Long serverId) throws BundleException, IOException {
        ...
        if (StringUtils.isNotEmpty(app.getAppThirdLocation())) {
            // 2、从服务器端下载应用包
            bundleLocation = downloadAppFromServer(id, app.getAppVersion(), app.getAppThirdLocation());
            // 3、下载应用后，将应用包安装到容器
    ...
    public String downloadAppFromServer(Long id, String appVersion, String appLocation) throws IOException {
        ...
        // 判断是否已安装过，且应用存在前台文件包
        if (appInstall != null && StringUtils.isEmpty(appInstall.getAppThirdLocation())) {
            return null;
        }
        // 判断是否已安装过，且版本与服务器版本一致
        if (appInstall != null && StringUtils.isNotEmpty(appInstall.getAppThirdLocation())) {
            File file = new File(appInstall.getAppThirdLocation());
            if (file.exists() && StringUtils.equals(appVersion, appInstall.getInstallVersion())) {
                return appInstall.getAppThirdLocation();
            }
        }
        Properties properties = PropertieUtil.readPropertiesFile(AppThirdServiceImpl.class.getClassLoader().getResourceAsStream(AppConstant.APP_PROP_LOCATION));
        URL url;
        URLConnection conn;
        ...
        try {
            ...
            // 下载地址
            url = new URL(serverUrl + "downloadapp.htm?id=" + id + "&appType=2&appMarketKey=" + marketKey.getAppMarketKey());
    ...
    public Bundle installApp(Long id) {
        ...
        Bundle bundle = null;
        try {
            bundle = Activator.getBundleContext().installBundle("file:" + appInstall.getAppThirdLocation());
    ...
    public Bundle startApp(Long id, Long serverId) {
        ...
        try {
            ...
            LOGGER.info("商家应用" + appInstall.getAppName() + "正在启动");
        } catch (BundleException e) {
            // 7、将应用的状态改为2，表示已安装未启动
            appInstall.setInstallStatus("2");
            LOGGER.error("商家应用启动失败", e);
        }
        // 8、更新bundle的运行状态
        appInstallService.update(appInstall);
        return bundle;
    ...
    public Bundle stopApp(Long id, Long serverId) {
        ...
        try {
            ...
            // 7、将应用的状态改为2，表示停止
            appInstall.setInstallStatus("2");
            LOGGER.info("商家应用" + appInstall.getAppName() + "正在停止");
        } catch (BundleException e) {
            // 7、将应用的状态改为3，表示还在启动状态
            appInstall.setInstallStatus("3");
            LOGGER.error("商家应用停止失败", e);
        }
        // 8、更新bundle的运行状态
        appInstallService.update(appInstall);
        return bundle;
    ...
    public Bundle uninstallapp(Long id, Long serverId) {
        ...
        try {
            ...
            appInstallService.update(appInstall);
            LOGGER.info("商家应用" + appInstall.getAppName() + "正在卸载");
        } catch (BundleException e) {
            LOGGER.error("商家应用卸载失败", e);
        }
        return bundle;
    }
