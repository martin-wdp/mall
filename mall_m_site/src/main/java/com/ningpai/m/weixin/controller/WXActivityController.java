...
public class WXActivityController {
     ...
     */
    //@RequestMapping("gettokentoevent")
    public String getWXOpenid(Pay pay,HttpServletRequest request, HttpServletResponse response) {
        // 获取code
        String code = request.getParameter("code");
        if (code == null) {
            return "";
        }
        LOGGER.error("微信返回值CODE"+code);
        // 获取微信接口AppId...
        String url = "https://api.weixin.qq.com/sns/oauth2/access_token?appid=" + pay.getApiKey() + "&secret=" + pay.getSecretKey() + "&code=" + code
                + "&grant_type=authorization_code";
        GetMethod getTokenMethod = new GetMethod(url);
        HttpClient clientToken = new HttpClient();
        getTokenMethod.getParams().setContentCharset(ConstantUtil.UTF);
        ...
        try {
            ...
            if (resultMap == null) {
                try {
                    ...
                    // 获取token失败...
                    OperaLogUtil.addOperaException("Getting token failed!", e, request);
                    LOGGER.error("Getting token failed!"+e.getMessage());
                    return "";
                }
            }
            try {
                return resultMap.get(ConstantUtil.OPENID);
            } catch (Exception e) {
                // 添加微信用户到群发组失败...
                OperaLogUtil.addOperaException("add wxuser to group failed!", e, request);
                LOGGER.error("add wxuser to group failed!"+e.getMessage());
                return "";
            }
            ...
            // 发送获取token请求失败
            OperaLogUtil.addOperaException("Sending getwxtoken request failed!", e, request);
            LOGGER.error("Sending getwxtoken request failed!"+e.getMessage());
            return "";
        }
    }
