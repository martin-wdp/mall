...
import com.ningpai.m.customer.vo.ExpressNoVo;
...
public class CustomerOrderControllerM {
    ...
    private SysLogisticsCompanyService selectFreightTemplateDetail;
    ...
    public ModelAndView queryAllOrders(HttpServletRequest request, PageBean pb, String date, String type, String paramString) {
        ...
        try {
            ...
            if (LoginUtil.checkLoginStatus(request)) {
                ...
                mav.addObject(ConstantUtil.PAGENAME,"我的订单");
            } else {
        ...
        try {
            if (LoginUtil.checkLoginStatus(request)) {
                OrderInfoBean orderInfoBean =
                        (OrderInfoBean)customerOrderService.queryOrderByCustIdAndOrderId(orderId, (Long) request.getSession().getAttribute(CustomerConstants.CUSTOMERID));
                resultMap.put("order",orderInfoBean );
                if(orderInfoBean.getExpressno()!=null&&orderInfoBean.getExpressno().size()>0){
                   for(ExpressNoVo expressNoVo:orderInfoBean.getExpressno()){
                        //查询物流跟踪信息 获得返回的URL
                       SysLogisticsCompany sysLogisticsCompany = selectFreightTemplateDetail.selectCompanyById(expressNoVo.getOrderExpressId());
                       if (sysLogisticsCompany!=null) {
                           expressNoVo.setExpressUrl(KuaiDiUtil.execLookKuaiDiNew(sysLogisticsCompany.getKuaidi100Code(), expressNoVo.getExpressNo()));
                          // expressNoVo.setExpressUrl(KuaiDiUtil.execLookKuaiDiNew("yuantong", "806278485939"));
                       }
                   }
                }
                mav = new ModelAndView(CustomerConstants.ORDERDETAIL).addAllObjects(resultMap);
                mav.addObject(ConstantUtil.PAGENAME,"订单信息");
                mav.addObject(ConstantUtil.BACKLEVURL, "customer/myorder.html?tag=4");
            } else {
                mav = new ModelAndView(CustomerConstants.REDIRECTLOGINTOINDEX);
            }
            return seoService.getCurrSeo(mav);
    ...
    public ModelAndView toShare(HttpServletRequest request, Long orderId) {
        ...
        try {
            ...
            mav.addObject(ConstantUtil.PAGENAME,"商品评价");
            return seoService.getCurrSeo(mav);
        } finally {
            mav = null;
            resultMap = null;
    ...
    public ModelAndView cancelOrder(HttpServletRequest request, PageBean pb, Long orderId, String reason) {
        ...
        try {
            // 检查用户是否登录
            if (LoginUtil.checkLoginStatus(request)) {
                customerOrderService.cancelOrder(request,orderId, reason);
