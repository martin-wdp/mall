...
public class CustomerOrderServiceImpl implements CustomerOrderService {
     ...
     */
    @Override
    public int cancelOrder(HttpServletRequest request,Long orderId, String reason) {
        int result = 0;
        Map<String, Object> paramMap = new HashMap<String, Object>();
        try {
            paramMap.put(ConstantUtil.ORDERID, orderId);
            ...
            if(result == 1){
                ...
                if(order.getOrderIntegral() != null){
                    //取消订单，将使用的积分返还到账号中
                    CustomerAllInfo customerAllInfo = customerMapper.selectByPrimaryKey((Long) request.getSession().getAttribute(CUSTOMERID));
                    customerAllInfo.setInfoPointSum(customerAllInfo.getInfoPointSum() + order.getOrderIntegral().intValue());
                    CustomerPoint customerPoint = new CustomerPoint();
                    customerPoint.setPointSum(customerAllInfo.getInfoPointSum().longValue());
                    customerPoint.setCustomerId(((Long) request.getSession().getAttribute(CUSTOMERID)));
    ...
    @Override
    public int addNewGoodsComment(HttpServletRequest request, Comment comment, Long orderId) {
        // 添加评论 返回评论编号
        comment.setIsConsult("0");
        comment.setPublishTime(new Date());
        comment.setIsDisplay("1");
        ...
        //customerOrderMapper.addGoodsComment(comment);
        int commenid =commentServiceMapper.addGoodsComment(comment);
        // 添加评论积分
        customerPointServiceMapper.addIntegralByType(comment.getCustomerId(), "4");
        // 修改订单商品为已评价 并更新评论编号
        GoodsBean goodsBean = new GoodsBean();
