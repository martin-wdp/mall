...
function register(){
    var temp = $("input[type='checkbox']").is(':checked');
    var customerUsername = $("input[name='customerUsername']").val();
    if(temp == true){
        $(".reg").bind("click",function(){
            var num=0;
            var flag= validateChkFun.formSubmit(['#mc','#mob','#pd']);
            if(flag&&num==0){
                if($("#pdt").val()==$("#pd").val()){
                    ...
                    $.ajax({
                        type: 'post',
                        url: "validate/getMCode.htm?code="+checkCode+"&mobile="+customerUsername,
        ...
        }).css("background","#F6AB00");
    }else{
        $(".reg").unbind("click").css("background","#FACD66");
    }
}
