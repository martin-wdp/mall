       ...
       <#-- <#assign productPrice=product.goodsInfoPreferPrice>-->
        ...
        <#--<div class="col-xs-6">
            <a href="${basePath}/item/${product.goodsInfoId}.html">
                ...
                <span style="text-align:left;height:2.6em;">${product.goodsInfoName!''}</span>
                <span>￥${productPrice?string('0.00')}</span>
            </a>
        </div>-->
        <li>
            <a href="${basePath}/item/${product.goodsInfoId}.html">
                <img src="<#if  product.imgList?? && product.imgList?size &gt; 0 >${product.imgList[0].imageBigName!''}</#if>">
                <p>${product.goodsInfoName!''}</p>
                <span>￥<strong>${productPrice?string('0.00')}</strong></span>
            </a>
            <div class="goodsBtn"><i data-productId="${product.goodsInfoId}" data-url="${basePath}/saveAtte.htm"
                <#if goodsIds?? && goodsIds?seq_contains('${product.goodsInfoId?string}')>
                        style="background-image: url('images/qp_sca.png')"
                        <#else >style="background-image: url('images/qp_scb.png')"
                </#if>></i><button
