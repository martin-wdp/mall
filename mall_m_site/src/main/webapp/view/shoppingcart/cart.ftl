        ...
        function oncheck() {
            ...
            for (var i = 0; i < obj.length; i++) {
                if ($(obj[i]).attr("c-status") == 0) {
                    ...
                    price += parseFloat(s) * parseInt(n);
                    /*accAdd(price,accMul(s,n));*/
                }
            }
...
<div class="cart_list">
    <!--<div class="goods_filter container-fluid">
        <div class="col-xs-6">
            <a val="5D" attr="" class="change_sort cur" href="javascript:;">全部商品(1)</a>
        </div>
        <div class="col-xs-6">
            <a val="2D" attr="" class="change_sort  " href="javascript:;">降价商品(0)</a>
        </div>
    </div>-->
            ...
            <#if cart.goodsDetailBean??>
                ...
                <div id="activity${cart.shoppingCartId}" class="cart_good cart_good_hover cart_item">
                    ...
                    <#if cart.goodsDetailBean.productVo.goodsInfoStock!=0&&cart.goodsDetailBean.productVo.goodsInfoDelflag='0' &&  cart.goodsDetailBean.productVo.goodsInfoAdded!='0'>
                        <a class="check_btn" name="boxs" cid="${cart.shoppingCartId}" goodinfoid="${cart.goodsDetailBean.productVo.goodsInfoId}" c-status="0"
                           id="check_btn${cart.shoppingCartId}" onclick="checked(${cart.shoppingCartId})"></a>
                    </#if>
                    <div class="img">
                        <a href="${basePath}/item/${cart.goodsDetailBean.productVo.goodsInfoId}.html" target="_blank" title="${basePath}/item/${cart.goodsDetailBean.productVo.goodsInfoId}.html">
                            <img title="${cart.goodsDetailBean.productVo.productName}" alt="${cart.goodsDetailBean.productVo.productName}"
                                 src="<#if cart.goodsDetailBean.productVo.goodsInfoImgId??>${cart.goodsDetailBean.productVo.goodsInfoImgId}</#if>"/>
                        </a>
                    </div>
                    <div class="word">
                        <p>
                        ...
                        <p class="price"><em>单价：</em>￥
                            <#if cart.marketing??>
                                <#assign codeType="${cart.marketing.codexType}"/>
                                <#if vip?? && vip=="1">
                                        <#if codeType=="10"><!-- 团购-->
                                    ${(cart.marketing.groupon.grouponVipPrice?number)?string("0.00")}
                                        <#elseif codeType=="11"><!-- 抢购-->
                                    ${(cart.marketing.rushs[0].rushVipPrice?number)?string("0.00")}
                                        <#elseif codeType=="15"><!-- 折扣-->
                                    ${(cart.marketing.preDiscountMarketings[0].vipdiscountPrice?number)?string("0.00")}
                                    <#else>
                                    ${(cart.goodsDetailBean.productVo.goodsInfoVipPrice?number)?string("0.00")}
                                    </#if>
                                <#else>
                                        <#if codeType=="10"><!-- 团购-->
                                    ${(cart.marketing.groupon.grouponPrice?number)?string("0.00")}
                                        <#elseif codeType=="11"><!-- 抢购-->
                                    ${(cart.marketing.rushs[0].rushPrice?number)?string("0.00")}
                                        <#elseif codeType=="15"><!-- 折扣-->
                                    ${(cart.marketing.preDiscountMarketings[0].discountPrice?number)?string("0.00")}
                                    <#else>
                        ...
                        <input type="hidden" class="productPrice" value="
                            <#if cart.marketing??>
                                <#if vip?? && vip=="1">
                                    <#if codeType=="10">
                                         ${(cart.marketing.groupon.grouponVipPrice?number)}
                                    <#elseif codeType=="11">
                                         ${(cart.marketing.rushs[0].rushVipPrice?number)}
                                    <#elseif codeType=="15">
                                         ${(cart.marketing.preDiscountMarketings[0].vipdiscountPrice?number)}
                                    <#else>
                                        ${(cart.goodsDetailBean.productVo.goodsInfoVipPrice?number)}
                                    </#if>
                                <#else>
                                    <#if codeType=="10">
                                        ${(cart.marketing.groupon.grouponPrice?number)}
                                    <#elseif codeType=="11">
                                        ${(cart.marketing.rushs[0].rushPrice?number)}
                                    <#elseif codeType=="15">
                                        ${(cart.marketing.preDiscountMarketings[0].discountPrice?number)}
                                    <#else>
                                            ${(cart.goodsDetailBean.productVo.goodsInfoPreferPrice?number)}
                                    </#if>
                                </#if>
                            <#else>
                                <#if vip?? && vip=="1">
                    ...
                    <div class="count">
                        ...
                        <input type="text" id="goods_num_${cart.shoppingCartId}"
                               onblur="opblur(this,${cart.shoppingCartId},${cart.goodsDetailBean.productVo.goodsInfoStock});"
                               class="count_num productNum" value="<#if cart.goodsNum??>${cart.goodsNum}</#if>">
                        <input type="hidden" id="goods_onb_${cart.shoppingCartId}" class="count_num productNum"
                               value="${cart.goodsDetailBean.productVo.goodsInfoStock}">
                ...
                <#--<a class="del" onclick="dodel(${cart.shoppingCartId},${cart.goodsDetailBean.productVo.goodsInfoId})"
                   href="javascript:"><span class="glyphicon glyphicon-trash"></span></a>-->
                </div>
                <!--商品end-->
            </#if>
        </#list>
        <div class="writeGoods" data-value="取消" num="1">编辑</div>
    <#else>
        ...
        <div class="cart_check container-fluid">
            <div class="row">
                <div class="col-xs-7">
                    <p class="fl">
                        <a href="javascript:" onclick="chenckAll(1)" class="all_check" style="margin-left: 1em;"><i></i>全选</a>
                        <a href="javascript:" onclick="chenckAll(0)" class="all_not_check"><i></i>反选</a>
                    </p>
                    <p class="total fl oper"><strong style="color:#333;font-weight:700;font-size:16px;">合计：<span id="suprice" style="color:red;"></span></strong></p>
                </div>
                <div class="col-xs-5">
                    <a href="javascript:" onclick="onpay()" class="go_check oper">去结算</a>
                    <div class="operate dis">
                        <a id="remove" onclick="delcBacht()" style="border:1px solid red;color:red;">删除</a><#if cust??><a id="move" onclick="moveToAtte()">移入收藏</a></#if>
                    </div>
                </div>
            </div>
        </div>
...
<div class="wrap">
    ...
    <div class="go_buy">
        <button type="button" class="btn btn-warning btn-lg "
                style="width:45%;float:left;background:#F6AB02;border:none;" onclick="goShop()">去逛逛
        </button>
        <button type="button" class="btn btn-warning btn-lg "
                style="width:45%;float:right;background:#BBB9BA;border:none;" onclick="goMyFollow()">查看收藏
        </button>
    </div>
</div>
</form>
...
<div class="foot" style="padding: 0.5em 0 9em 0;">
...
<script>
    ...
    $(".writeGoods").click(function(){
        var text = $(this).text();
        var text2 = $(this).attr("data-value");
        $(this).text(text2).attr("data-value",text);
        if($(this).attr("num") == "1"){
            $(".oper").hide();
            $(".operate").show();
            $(".smart_menu").hide();
     ...
     *
     */
    function addAttr(infoId) {

        $.post("${basePath}/checkAtte.htm?productId=" + infoId, function (data) {
            if (!data) {
                $.post("${basePath}/saveAtte.htm?productId=" + infoId, function (data) {
                    alert(data)
                });
            }
        });
    ...
    function goMyFollow() {
        location.href = "${basePath}/customer/myfollw.htm";
    }
    ...
    function delcBacht(){
        ...
        if (obj != null && obj.length != 0) {
            for (var i = 0; i < obj.length; i++) {
                if ($(obj[i]).attr("c-status") == 0) {
                    delcNew($(obj[i]).attr("cid"),$(obj[i]).attr("goodinfoid"));
                }
            }
        }
    }
    /**
     * 移入收藏
     * 逻辑：被选中的商品首先是在购物车中删除，然后判断在该商品是否被收藏，若已经
     * 收藏则不操作，若没有就收藏就添加收藏
     */
    function moveToAtte(){
        var obj = document.getElementsByName('boxs');
