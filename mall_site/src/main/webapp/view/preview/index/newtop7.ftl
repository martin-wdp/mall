<#assign basePath=request.contextPath>
<input type="hidden" id="basePath" value="${basePath}"/>
<div class="section_headerTop">	
    <div class="slot slot_headerTop01">
        <ul class="siteNav right">
            <li><span>您好<#if cust??>${cust.customerNickname!''}</#if>，欢迎来到${topmap.systembase.bsetName}</span></li>
            <li class="login_info">
            	<#if cust??>
            	<#else>
                <a class="site_login" href="${basePath}/login.html">[请登录]</a>
                <a class="site_register" href="${basePath}/register.html">[免费注册]</a>
                </#if>
            </li>
            <li class="loginout_info">
            	<#if cust??>
                <a class="site_loginout" href="${basePath}/logout.html">退出</a>
                </#if>
            </li>
            <li class="nav_item navCenter">
                <a href="${basePath}/customer/index.html">会员中心<s></s></a>
                <div class="navInfo">
                    <a href="${basePath}/customer/myorder.html">我的订单</a>
                </div>
            </li>
            <li class="nav_item"><a href="javascript:;"  onclick="addToFavorite('${topmap.systembase.bsetName}');">加入收藏</a></li>
        </ul>
    </div>
    <div class="slot slot_headerTop02"></div>
</div>