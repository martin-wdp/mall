...
public class OrderMServiceImpl extends BasicSqlSupport implements OrderMService {
    ...
    public Order getThirdOrderPrice(Long thirdId, String codeNo, Long amount, Long customerId, String orderCode, Long cityId, Long distinctId, List<ShoppingCart> shoplist,
        ...
        Map<String, Object> map = shoppingCartService.getEveryThirdPriceMap(thirdId, shoplist, distinctId);
        // 订单交易总金额
        ...
        if (typeIdflag) {
            List<ShoppingCart> nobaoyou = shoppingCartService.getNobaoyouShoppingcarts(shoplist);
    ...
    public List<Order> newsubmitOrder(Long duiHuanJiFen, String invoiceType, String invoiceTitle, HttpServletRequest request, Long[] shoppingCartId, Long typeId,
        ...
        for (int i = 0; i < cartlist.size(); i++) {
            if (cartlist.get(i).getFitId() == null) {
                ...
                // 如果商品是套装
                GoodsGroupVo goodsGroupVo = goodsGroupService.queryVoByPrimaryKey(((ShoppingCart) cartlist.get(i)).getFitId());
            ...
            if (cartlist != null && !cartlist.isEmpty()) {
                ...
                for (int i = 0; i < cartlist.size(); i++) {
                    if (cartlist.get(i).getFitId() != null) {
                        ...
                        // 查询商品详细
                        cartlist.get(i).setGoodsDetailBean(
                                siteGoodsProductService.queryDetailBeanByProductId(((ShoppingCart) cartlist.get(i)).getGoodsInfoId(),
                                        ((ShoppingCart) cartlist.get(i)).getDistinctId()));
                        // 查询购物车里选择的促销信息
                        cartlist.get(i).setMarketing(marketService.marketingDetail(cartlist.get(i).getMarketingActivityId(), cartlist.get(i).getGoodsInfoId()));
                        ...
                        if (thirdId.equals(cartlist.get(i).getGoodsDetailBean().getProductVo().getThirdId())) {
                            ...
                            if (thirdId == 0 && distinctId != null) {
                                ...
                                if (productWare != null) {
                                    // 设置商品库存
                                    ((ShoppingCart) cartlist.get(i)).getGoodsDetailBean().getProductVo().setGoodsInfoStock(productWare.getWareStock());
                                    ((ShoppingCart) cartlist.get(i)).getGoodsDetailBean().getProductVo().setGoodsInfoPreferPrice(productWare.getWarePrice());
                    ...
                    if (cartlist.get(i).getFitId() != null) {
                        ...
                        if (thirdId.equals(cartlist.get(i).getGoodsDetailBean().getProductVo().getThirdId())) {
                            ...
                            if (productWare != null) {
                                ...
                                // 如果没有，则设置库存为0
                                ((ShoppingCart) cartlist.get(i)).getGoodsDetailBean().getProductVo().setGoodsInfoStock(ShopCartValueUtil.WARECOUNT);
                            }
                            OrderGoods og = new OrderGoods();
                            og.setGoodsInfoNum(cartlist.get(i).getGoodsNum());
                            og.setDelFlag("0");
