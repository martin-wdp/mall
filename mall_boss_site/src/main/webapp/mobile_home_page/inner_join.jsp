        ...
        $.ajax({
            ...
            success : function(data){
                ...
                for(var i=0;i<data.length;i++){
                    ...
                    +'<td>'
                    +'<span class="link">'+'list/'+data[i].cateId+'</span></td>'
                    +'<td class="tr">';
                    if(data[i].temp3==1){
                    ...
                    if(data[i].childs.length>0){
                        for(var j=0;j<data[i].childs.length;j++){
                            html = html + '<tr class="level-2">'
                            +'<td>'+data[i].childs[j].name+'</td>'
                            +'<td><span class="link">'+'list/'+data[i].childs[j].cateId+'</span></td>'
                            +'<td class="tr"><button class="ct-choose" onclick="cc(this);">选择</button></td></tr>';
                        }
                    }
                    $("#cate").find("tbody").append(html);
                }
...
//						html = html +'</td><td><span class="link">item/'+goods.goodsInfoId+'.html</span></td>'
