$(function(){
	...
	$("#fullRollChoose").click(function(){
		...
		if(num>0){
			art.dialog.open($("#basePath").val()+'queryImageManageByPbAndCidForChoose.htm?CSRFToken='+$("#CSRFToken").val()+'&size='+num, {
				id: 'fullRoll',
				lock: true,
				width: '800px',
				height: '400px',
...
//		newSlide.append();
		...
		$(".fullRollAgain").click(function(){
			art.dialog.open($("#basePath").val()+'queryImageManageByPbAndCidForChoose.htm?CSRFToken='+$("#CSRFToken").val(), {
				id: $(this).prop("id"),
				lock: true,
				width: '800px',
				height: '400px',
