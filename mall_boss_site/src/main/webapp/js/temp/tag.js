...
function pageBeanTag(data){
    ...
    if(data.pb.pageNo==1){
        ...
        footStr+='<a  href="javascript:;" onclick="showHTagList('+(data.pb.pageNo-1)+')"> 上一页 </a>';
    }
    $(tagFootHtml).html("");
    for(var i = data.pb.startNo;i<data.pb.endNo+1;i++){
        if(i==data.pb.pageNo){
            ...
            footStr+='<a href="javascript:;"  onclick="showHTagList('+i+')">'+i+'</a>';
        }
    }
    if(data.pb.pageNo==data.pb.totalPages){
        footStr+='<span class="disabled"> 下一页 </span>';
    }else{
        footStr+='   <a href="javascript:;" onclick="showHTagList('+(data.pb.pageNo+1)+')"> 下一页 </a>';
    }
    ...
    footStr+=' <select class="form-control" id="showTagOpetion" onchange="showHTagList('+data.pb.pageNo+')">';
