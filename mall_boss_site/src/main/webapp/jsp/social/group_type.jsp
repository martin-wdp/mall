...
<div class="page_body container-fluid">
  <div class="row">
    ...
    <input type="hidden" value="groupTypeList" id="formId"/>
    <input type="hidden" value="grouptype.htm" id="formName"/>
        ...
        <div class="common_data p20">
            ...
            <form role="form" class="form-inline" id="search_from" method="post">
              ...
              <div class="form-group">
                <div class="input-group">
                  ...
                  <input type="text" class="form-control search_text" name="groupTypeName" value="${attr}">
                </div>
              </div>
              <div class="form-group">
          ...
          <div class="data_ctrl_area mb20">
            ...
            <div class="data_ctrl_brns pull-left">
              <button type="button" class="btn btn-info" onclick="$('#addGroupCate').modal('show')">
                <i class="glyphicon glyphicon-plus"></i> 添加
              </button>
              <button type="button" class="btn btn-info" id="stop-type">
                <i class="glyphicon glyphicon-remove-sign"></i> 停用
              </button>
              <button type="button" class="btn btn-info" id="recovery-type">
                <i class="glyphicon glyphicon-ok-sign"></i> 恢复
              </button>
            </div>
            <div class="clr"></div>
          </div>
          <form action="" id="groupTypeList" method="post">
          <table class="table table-striped table-hover">
            <thead>
            <tr>
              <th width="10"><input type="checkbox" onclick="allunchecked(this,'groupTypeId')"></th>
              <th>分类名称</th>
              <th>分类状态</th>
              <th>分类描述</th>
               <th>创建时间</th>
            ...
            <tbody>
            <c:forEach items="${pageBean.list}" var="grouptype" varStatus="i">
            <tr>
              <td><input type="checkbox" name="groupTypeId" value="${grouptype.groupTypeId }"></td>
              <td>${grouptype.groupTypeName}</td>
              <td>
                <c:if test="${grouptype.groupTypeStatus ==0}">
                  <span class="label label-success">正常</span>
                </c:if>
                <c:if test="${grouptype.groupTypeStatus ==1}">
                  <span class="label label-default">停用</span>
                </c:if>
              </td>
             
              <td>${grouptype.groupTypeRemark}</td>
               <td>
                <fmt:formatDate value="${grouptype.groupTypeCreateTime}" pattern="yyyy-MM-dd HH:mm:ss" var="createTime"/>
                  ${createTime}
              </td>
              <td>
                <fmt:formatDate value="${grouptype.groupTypeModifyTime}" pattern="yyyy-MM-dd HH:mm:ss" var="modifyTime"/>
                  ${modifyTime}
              </td>
              <td>
                <div class="btn-group">
                  <button type="button" class="btn btn-default" onclick="modifiedType('${grouptype.groupTypeId}');">编辑</button>
                  <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
                    <span class="caret"></span>
                    <span class="sr-only">Toggle Dropdown</span>
                  </button>
                  <ul class="dropdown-menu" role="menu">
                    <li><a href="javascript:;" onclick="stopType('disablegroup.htm','grouptype.htm','${grouptype.groupTypeId}');">停用</a></li>
                    <li><a href="javascript:;" onclick="recoveryType('recoverygroup.htm','grouptype.htm','${grouptype.groupTypeId}');">恢复</a></li>
                    <li><a href="javascript:;" onclick="delGroupType('${grouptype.groupTypeId}');">删除</a></li>
                  </ul>
                </div>
              </td>
            </tr>
...
<!-- Modal -->
<div class="modal fade" id="addGroupCate"  role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="updateTitle">添加小组分类</h4>
      </div>
      <div class="modal-body">
        <form class="form-horizontal" action="addgrouptype.htm?CSRFToken=${token}" id="dialog-form" method="post" enctype="multipart/form-data">
          <input type="hidden" name="groupTypeId" id="groupTypeId"/>
          <input id="oldgroupTypeName" type="hidden" />
          <div class="form-group">
            <label class="control-label col-sm-5"><span class="text-danger">*</span>分类名称：</label>
            <div class="col-sm-1"></div>
            <div class="col-sm-8">
              <input type="text" class="form-control required" name="groupTypeName" id="groupTypeName"/>
            </div>
          </div>
          <div class="form-group">
            ...
            <div class="col-sm-1"></div>
            <div class="col-sm-4">
              <input type="text" class="form-control  required number"  maxlength="3" name="groupTypeSort" id="groupTypeSort"/>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <label class="radio-inline">
                <input type="radio" id="status1" value="0" checked="checked" name="groupTypeStatus"/> 正常
              </label>
              <label class="radio-inline">
                <input type="radio" id="status2" value="1" name="groupTypeStatus"/> 停用
              </label>
            </div>
            <div class="col-sm-3"></div>
          </div>
          <div class="form-group">
            <label class="control-label col-sm-5">分类描述：</label>
            <div class="col-sm-1"></div>
            <div class="col-sm-16">
              <textarea class="form-control" rows="5" name="groupTypeRemark" id="groupTypeRemark"></textarea>
            </div>
            <div class="col-sm-3"></div>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-1"></div>
            <div class="col-sm-16">
              <textarea class="form-control" rows="5" name="seoRemark" id="seodesc"></textarea>
            </div>
            <div class="col-sm-3"></div>
          </div>
        </form>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-primary" onclick="addGroupCate();">确定</button>
...
<script src="<%=basePath %>js/social/grouptype.js"></script>
