              ...
              <form class="form-horizontal"  action="savefreight.htm?CSRFToken=${token}" method="post" id="freightForm">
               <input type="hidden" name="freightTemplateId" id="freightTemplateId" value="${freightTemplate.freightTemplateId }" />
                <div class="form-group">
                  ...
                  <div class="col-sm-16">
                    <input type="text" class="form-control w200 required" name="freightTemplateName" id="freightTemplateName" value="${freightTemplate.freightTemplateName }" >
                  </div>
                     		 ...
                     		 默认运费：
                       	 	<c:forEach items="${freightTemplate.freightExpressList}" var="expresslist"> 
								    ...
								    <input type="hidden" id="${company.code }_areas_n1" name="${company.code }_areas" value=""/>
							  		<input type="text" name="${company.code }_start"  <c:if test="${freightTemplate.freightPackageMail==1 }"> readonly="readonly" </c:if> value="${expresslist.expressStart }" id="${company.code }_start" class="w50"><span class="methods"><c:if test="${freightTemplate.freightMethods==0 }"> 件 </c:if><c:if test="${freightTemplate.freightMethods==1}"> g </c:if>内，</span>
						            <input type="text" name="${company.code }_postage" <c:if test="${freightTemplate.freightPackageMail==1 }"> readonly="readonly" </c:if> value="${expresslist.expressPostage }" id="${company.code }_postage" class="w50">元，  每增加
						            <input type="text" name="${company.code }_plus" <c:if test="${freightTemplate.freightPackageMail==1 }"> readonly="readonly" </c:if>  value="${expresslist.expressPlusN1 }" id="${company.code }_plus" class="w50"><span class="methods"><c:if test="${freightTemplate.freightMethods==0 }"> 件 </c:if><c:if test="${freightTemplate.freightMethods==1}"> g </c:if>， 增加运费</span>
						            <input type="text"  name="${company.code }_postageplus" <c:if test="${freightTemplate.freightPackageMail==1 }"> readonly="readonly" </c:if>  value="${expresslist.expressPostageplus }" id="${company.code }_postageplus"   class="w50">元
							   		</c:if>
						   		</c:forEach>    
						   		 <c:if test="${temp1==0 }">
						   		 	<input type="hidden" id="${company.code }_areas_n1" name="${company.code }_areas" value=""/>
              ...
              $("input[name='logComId']").each(function(){
                  if(this.checked==true){
                    ...
                    var $tempcode=$(this).attr("codename");
                      if(/^[0-9]+([.]{1}[0-9]+){0,1}$/.test($temp.find("input[name='"+$tempcode+"_start']").val())){
                      ...
                      if(/^[0-9]+([.]{1}[0-9]+){0,1}$/.test($temp.find("input[name='"+$tempcode+"_plus']").val())){
