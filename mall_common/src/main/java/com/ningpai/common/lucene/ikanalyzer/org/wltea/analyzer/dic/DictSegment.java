...
class DictSegment implements Comparable<DictSegment> {
    ...
    Hit match(char[] charArray, int begin, int length, Hit searchHit) {
        Hit searchHitNew = searchHit;
        if (searchHitNew == null) {
            // 如果hit为空，新建
            searchHitNew = new Hit();
            // 设置hit的其实文本位置
            searchHitNew.setBegin(begin);
        } else {
            // 否则要将HIT状态重置
            searchHitNew.setUnmatch();
        }
        // 设置hit的当前处理位置
        searchHitNew.setEnd(begin);
        ...
        if (ds != null) {
            if (length > 1) {
                // 词未匹配完，继续往下搜索
                return ds.match(charArray, begin + 1, length - 1, searchHitNew);
            } else if (length == 1) {

                // 搜索最后一个char
                if (ds.nodeState == 1) {
                    // 添加HIT状态为完全匹配
                    searchHitNew.setMatch();
                }
                if (ds.hasNextNode()) {
                    // 添加HIT状态为前缀匹配
                    searchHitNew.setPrefix();
                    // 记录当前位置的DictSegment
                    searchHitNew.setMatchedDictSegment(ds);
                }
                return searchHitNew;
            }
