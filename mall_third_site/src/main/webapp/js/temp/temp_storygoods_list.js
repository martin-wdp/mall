...
/**
 * 模板楼层商品js
 * Created by NingPai-liangck on 2015/5/22.
 */
...
function addProduct(pageNo,pageSize){
    ...
    $.post("queryProductForCoupon.htm?groupId=&pageNo="+pageNo+"&pageSize="+pageSize+"&brandIds="+brandId+"&productNo="+goodsInfoItemNo,function(data){
        var list=data.list;
...
function fillFormData(storeygoodsid){
    $.get("getChannelStoreyGoodsById.htm",{channelGoodsStoreyId:storeygoodsid},function(data){
        if(data!=null){
...
/*重置表单*/
function resetForm(){
    document.getElementById("storyGoodsInfoForm").reset();
    $("#up_barId").val("");
...
$(function(){
    ...
    /*模态框隐藏后，隐藏货品列表，*/
    /*$("#addFloor").on("hide.bs.modal",function(){
     $(".choose-goods").hide();
     $(this).text("选择货品");
     });*/
    ...
    $(".chooseGoods").click(function(){
        if($(".choose-goods").is(":hidden")){
            addProduct(1,5);
            $(".choose-goods").show();
            $("#storyGoodsInfoForm").css("display","none");
            $("#storyGoodsInfoForm").hide();
            $(this).text("关闭");
        }else{
            $(".choose-goods").hide();
            $("#storyGoodsInfoForm").show();
    ...
    /*添加樓層商品按鈕*/
    $(".create-floorgoods-btn").click(function(){
        $(".choose-goods").hide();
        $(".chooseGoods").html("选择货品");
        $("#storyGoodsInfoForm").show();
        /*重置表單*/
        resetForm();
        /*设置弹出框标题*/
        $(".storey-goodsinfo-title").html("添加货品");
        /*设置表单action*/
        $("#storyGoodsInfoForm").attr("action","createThirdTempStoreyGoods.htm");
    });

    /*修改楼层商品按钮*/
    $(".modify-storeygoods-btn").click(function(){
        /*重置表单*/
        resetForm();
        /*填充表單數據*/
        fillFormData($(this).attr("data-key"));
        /*設置彈出框標題*/
        $(".storey-goodsinfo-title").html("修改货品");
        /*設置表單action*/
        $("#storyGoodsInfoForm").attr("action","updateThirdTempStoreyGoods.htm");
    ...
    $("div.modal-footer").on("click","button.single-delete",function(){
        $("#singleDeleteForm").attr("action",$("#basePath").val()+"/deleteThirdTempStoreyGoods.htm").submit();
    });
    ...
    $("div.modal-footer").on("click","button.muilty-delete",function(){
        $("#muilt-delete-form").attr("action",$("#basePath").val()+"/deleteThirdTempStoreyGoods.htm").submit();
    });
