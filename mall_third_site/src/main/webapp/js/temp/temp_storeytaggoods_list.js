...
function validateForm(){
    ...
    if($("#up_goodsproductId").val()==""){
        $("#up_goodsproductNo").next().html("请选择商品!");
        flag = false && flag;
    }else{
 ...
 */
function resetForm(){
    document.getElementById("storeyTagGoodsForm").reset();
    $("#up_barId").val("");
...
/**
 * 填充表单数据
 * @param goodsId 标签货品ID
 */
function fillFormData(goodsId){
    $.get("getTagGoodsInfoById.htm",{tagGoodsId:goodsId},function(data){
        if(data!=null){
...
$(function(){
    ...
    /*添加按钮*/
    $(".create-storeytaggoods-btn").click(function(){
        /*重置表单*/
        resetForm();
        /*设置弹出框标题*/
        $(".storey-taggoods-title").html("添加货品");
        /*设置表单action*/
        $("#storeyTagGoodsForm").attr("action","createThirdTempStoreyTagGoods.htm");
    });

    /*修改按钮*/
    $(".modify-taggoods-btn").click(function(){
        /*重置表单*/
        resetForm();
        /*填充表单数据*/
        fillFormData($(this).attr("data-key"));
        /*设置弹出框标题*/
        $(".storey-taggoods-title").html("修改货品");
        /*设置表单action*/
        $("#storeyTagGoodsForm").attr("action","updateThirdTempStoreyTagGoods.htm");
        $("#addFloor").modal('show');
    });
    ...
    $("div.modal-footer").on("click","button.single-delete",function(){
        $("#singleDeleteForm").attr("action",$("#basePath").val()+"/deleteThirdTempStoreyTagGoods.htm").submit();
    });
    ...
    $("div.modal-footer").on("click","button.muilty-delete",function(){
        $("#muilt-delete-form").attr("action",$("#basePath").val()+"/deleteThirdTempStoreyTagGoods.htm").submit();
    });
