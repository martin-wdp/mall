...
<head>
    ...
    <script type="text/javascript" src="js/artDialog4.1.7/artDialog.source.js?skin=default"></script>
...
<div role="tabpanel" class="upload_body">
    <ul class="nav nav-tabs mb20" role="tablist">
        ...
        <li role="presentation"><a href="#tab2" aria-controls="tab2" role="tab" data-toggle="tab"
                                   onclick="doAjaxForCate(1, 5,${thirdId!''});">图库选择图片</a></li>
...
<script>
    ...
    /*执行AJAX查询分类信息*/
    function doAjaxForCate(pageNo, pageSize,thirdId) {
        /*AJAX查询分类信息*/
        $.get("ajaxQueryImageForChoose.htm?classifyId=-1&pageNo=" + pageNo + (pageSize == undefined ? "" : ("&pageSize=" + pageSize))+"&thirdId="+thirdId,
                function (data) {
                    //设置条数
                    ...
                    if (data.pageNo == 1) {
                        foot = foot + '<span class="disabled">&laquo;</span>';
                    } else {
                        foot = foot + '<a href="javascript:void(0)" onclick="doAjaxForCate(' + data.prePageNo + ',' + data.pageSize +',' + thirdId + ')">&laquo;</a>';
                    }
                    for (var l = data.startNo; l <= data.endNo; l++) {
                        if ((i - 1 + data.startNo) == data.pageNo) {
                            ...
                            foot = foot + "<a onclick='doAjaxForCate(" + (i - 1 + data.startNo) + "," + (data.pageSize) +"," + thirdId + ")' href='javascript:void(0)'>" + (i - 1 + data.startNo) + "</a>";
                        }
                        i++;
                    }
                    foot = foot + "<a onclick='doAjaxForCate(" + data.nextPageNo + "," + data.pageSize + "," + thirdId + ")' href='javascript:void(0)'>&raquo;</a></div></div>";
